...
float4 ReadLayerMap(int iSampler, sampler Sampler, float4 vColor)
{
	float4 vUV =0;
	...
	return tex2D( Sampler, vTexcoord ) *vColor;
}
...
shared float4 g_vLayerColor[3];

void PSMAIN( inout PSOUTPUT output)
{
	float4 vLayer0Color = ReadLayerMap(0, DiffuseMapLayer0, g_vLayerColor[0]) * g_vDiffuseLayerOpacity.x;
	if(g_bUseMaskMap.x)
		vLayer0Color *= ReadLayerMap(3, DiffuseMapMask0, 1);

	float4 vLayer1Color = ReadLayerMap(10, DiffuseMapLayer1, g_vLayerColor[1]);
	float4 vLayer1Mask = g_bUseMaskMap.y ? ReadLayerMap(11, DiffuseMapMask1, 1) : 1;
