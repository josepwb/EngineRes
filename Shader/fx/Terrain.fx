...
void SetTexCoordSplattingOnepass( inout VSOUTPUT output, in float4 vWorldPosition, in VSINPUT input, bool bUseNormalmap)
{
...
#endif
	// 마스킹 텍스쳐
	output.vAlphaCoord		= float2(input.vNormal.w, input.vTangent.w) * g_alphaTextureScaleAndTranslate.xy;
	...
	if( bUseNormalmap)
	{		
		float3 vNor = input.vNormal.xyz;
		float3 vTan = input.vTangent.xyz;
		vTan.xy *= -1;
		float3 vBin = normalize( cross( vTan, vNor)) * -1;
		float3 vNormal = normalize( mul( vNor, (float3x3)g_matWorldView));
		float3 vTangent = normalize( mul( vTan, (float3x3)g_matWorldView));
		float3 vBinormal = normalize( mul( vBin, (float3x3)g_matWorldView));

		output.vTangent = vTangent;
		output.vBiNormal = vBinormal;
		output.vLocalNormal = vNormal;
	}
#endif	// USE_NORMALMAP
}
...
void CalcTBN_BasedNormal( inout float3 _vOutNormal, in float4 _vNormalColor, in float3 _vNormal, 
in float3 _vBinormal, in float3 _vTangent )
{
	...
	_vOutNormal = normalize( mul( _vNormalColor, TangentToView));	
}
...
void CalcSplattingOnepass( out float4 vDiffuseColor, out float fSpecular, inout float3 vNormal, in VSOUTPUT input)
{
	...
	CalcTBN_BasedNormal( vNormal, vNormalColor, input.vLocalNormal, input.vBiNormal, input.vTangent );
#endif /// USE_NORMALMAP
...
#elif RT2 == 1
	output.color =  float4(0.01f, 0, 0, 1);
	output.color1 =  float4(0.01f, 0, 0, 1);
#elif RT3 == 1
	output.color =  float4(0.01f, 0, 0, 1);
	output.color1 =  float4(0.01f, 0, 0, 1);
	output.color2 =  float4(0.01f, 0, 0, 1);
#endif
