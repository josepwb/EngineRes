...
shared float	g_fDoubleShadowIntensity = 0.15f;


// PSSM Shadow
#define MAX_SPLIT_COUNT 4
shared float3 	g_vCSMSplitCenter[MAX_SPLIT_COUNT];	// xyz = center, w = radius
shared float4 	g_vCSMSplitRadius;	// xyz = center, w = radius
...
//Ambient Light
shared float4x4 g_matWorldViewInv;
shared float4 g_vAmbientLightRange;
...
float2 IsShadow(float2 vecCoord, float d)
{
...
void IsShadow_Poisson(float2 vecCoord, float d, out float2 outfShadow)
{
    ...
    for(int i = 0; i < POISSON_COUNT; ++i)
    {
		...
		float2 fShadow = IsShadow(shadowMapUV, d);
...
float GetSpecular(float fSpecularCompress, float3 vView, float3 vViewNormal, float3 vLight)
{
	...
	return (pow ( max(dot(vReflect, vLight), 0), fSpecularPower ) * fSpecularScale)
#ifdef SKY_SPECULAR
...
float GetRim(float3 vViewNormal, float3 vView, float fViewPosZ)
{
	float cosView = dot(vViewNormal, vView);
	...
	return fRim * rimScale * g_fRimScale;
}


float2 GetShadowESM(float4 vViewPos, float3 vViewNormal)
{
	if( distance(vViewPos.xyz, g_vCSMSplitCenter[0]) > g_vCSMSplitRadius.x )
		return 1;

	float4 vDepthCoord	= 0;
	float4 vBiasedPosition = vViewPos;
	vBiasedPosition.xyz += vViewNormal*g_vDepthBias[0];

	float4 vShadowRefUV = mul(vBiasedPosition ,g_lightTransformArray[0]);
	vShadowRefUV /= vShadowRefUV.w;    
	float fShadowCompareD = dot(vBiasedPosition, g_lightViewTransformArray[0]);

	float2 fShadow=1;
	#ifdef SHADOW_FILTER_POISSON
		IsShadow_Poisson_Half( float4(vShadowRefUV.xy,0,1), fShadowCompareD, fShadow );
	#else
		fShadow = IsShadow( float4(vShadowRefUV.xy,0,1), fShadowCompareD);
	#endif
...
{
	float4 vBiasedPosition = vViewPos;
	vBiasedPosition.xyz += vViewNormal*g_vDepthBias[nSplitPosition];
...
{
	float4 fDist=0;
	fDist.x = distance(vViewPos.xyz, g_vCSMSplitCenter[0]);
	fDist.y = distance(vViewPos.xyz, g_vCSMSplitCenter[1]);
	fDist.z = distance(vViewPos.xyz, g_vCSMSplitCenter[2]);
	fDist.w = distance(vViewPos.xyz, g_vCSMSplitCenter[3]);

	float4 fRadius = g_vCSMSplitRadius;
	int4 bInSplit= fRadius>=fDist;

	int4 Mask = bInSplit * float4(4,3,2,1);
	int nPos = 4-max( max(Mask.x, Mask.y), max(Mask.z, Mask.w) );
	if(nPos==4)
		return 1;
	
	int4 CheckPos0 = int4(0, 1, 2, 3);
	int4 CheckPos1 = int4(bInSplit.yzw+int3(0,1,2), 3);

	int2 nSplitPosition=-1;
	nSplitPosition.x = CheckPos0[nPos];
	nSplitPosition.y = CheckPos1[nPos];

	float fBlendArea = 0.2f;
	float fCSM_LerpTerm =fDist[nSplitPosition.x] / fRadius[nSplitPosition.x]; 	
	fCSM_LerpTerm = (((1.0f-fCSM_LerpTerm)/fBlendArea));


	float2 fShadow = GetSplitShadow(vViewPos, vViewNormal, nSplitPosition.x);
					 ...
					 out float outfShadow, out float outfDuplicatedShadow, out float outShadowExceptCloud)
{
	...
	outShadowExceptCloud = min(vShadow.x, vShadow.y);	// 구름을 적용 하지 않은 상태의 그림자
	...
	#ifdef CLOUD
	{
		...
		vShadow.x = min( vShadow.x, vCloudColor.x);
	}
	#endif

	outfShadow =min(vShadow.x, vShadow.y);

	outfDuplicatedShadow = fLighting * (1-vShadow.y) * (1-vShadow.x);	// 원래 값의 inv가 된다. 0이면 그림자가 없는 상태
}


float3 GetAttenuation(float3 vStart, float3 vEnd, float3 vLightLength, float fVisibility)
{
	float3 vAttenuationRange = vEnd - vStart;
	float3 vAttenuation = (vEnd - vLightLength) / vAttenuationRange;
	return saturate(vAttenuation) *fVisibility;
}
...
void ApplyDoubleShadow(in float fDuplicatedShadow, inout float3 vResultColor)
{
	...
	vResultColor -= (fDuplicatedShadow) * g_fDoubleShadowIntensity * bDuplicatedShadow;
	vResultColor = max(vResultColor, 0.0f);
}



float4 GetLightingComplexityColor(float fAttenuation)
{
	return float4(0.1f,0,0,1.0f) * (fAttenuation>0);
}
...
float4 PSMAIN(VSOUTPUT input) : COLOR
{
...
#ifdef COMPLEXITY
	return GetLightingComplexityColor(1)*(SUB_LIGHT_COUNT+1);
#endif
	...
	float fLighting = dot(vLight, vViewNormal);
	...
	float fRim = 0;
			...
			fSpecular = GetSpecular(abs(vDepthSpec.y), vView, vViewNormal, vLight);
				...
				[branch]
				if(vDepthSpec.y>0)
					fRim = GetRim(vViewNormal, vView, vPosition.z);
			#endif
		#endif
	#endif
	//-----------------------------------------------------------------------------------------
	...
	float3 vResultColor = fLightingWithShadowResult*g_vLightColor +GetSubLightingResult(vViewNormal) +fRim +g_vLightAmbient;
...
#if defined(POINT_LIGHT_AREA_RECTANGLE) || defined(POINT_LIGHT_AREA_ELLIPSE)
	#define AREA_LIGHT
#endif
...
#ifdef POINT_LIGHT_AREA_RECTANGLE
	float3 vPosition = input.vPosition.xyz * (vRadius+vLightAreaRange*0.5f);
#elif POINT_LIGHT_AREA_ELLIPSE
	float3 vPosition = input.vPosition.xyz * vLightAreaRange*0.5f;
#else
...
float4 PSMAIN_POINTLIGHT( VSOUTPUT_POINTLIGHT input, float2 vPos : VPOS) : COLOR
{
	...
	#ifdef POINT_LIGHT_AREA_RECTANGLE
		float3 vLightHalfRange = fLightRange/2.0f;

		float fX = clamp( dot(-vLight,input.vAxisX), - vLightHalfRange.x, vLightHalfRange.x);
		float fY = clamp( dot(-vLight,input.vAxisY), - vLightHalfRange.y, vLightHalfRange.y);
		float fZ = clamp( dot(-vLight,input.vAxisZ), - vLightHalfRange.z, vLightHalfRange.z);

		float3 vRealLightPosition = input.vLightPos + input.vAxisX*fX + input.vAxisY*fY +input.vAxisZ*fZ;

		vLight = vRealLightPosition-vPosition.xyz;
	#elif POINT_LIGHT_AREA_ELLIPSE
		float fX = dot(vLight,input.vAxisX);
 		float fY = dot(vLight,input.vAxisY);
 		float fZ = dot(vLight,input.vAxisZ);

		fRadius = min(min(fLightRange.x,fLightRange.y), fLightRange.z);

		vLight = (float3(fX,fY,fZ)*fRadius) / (fLightRange*0.5f);
		vLight = input.vAxisX*vLight.x + input.vAxisY*vLight.y + input.vAxisZ*vLight.z;	
	#endif

	float fLightDistance = length(vLight);
	#ifdef POINT_LIGHT_AREA_RECTANGLE
		clip( fLightDistance-0.01f );
	#endif
	clip(fRadius - fLightDistance);
#endif
...
#ifdef COMPLEXITY
	return GetLightingComplexityColor(fAttenuation);
#else
	//-----------------------------------------------------------------------------------------
	//Specluar
	float fSpecular = 0;
...
float4 PSMAIN_SHADOWRECEIVER(float2 vTexcoord : TEXCOORD0) : COLOR
{
	...
	if( distance(vPosition.xyz, g_vCSMSplitCenter[0]) > g_vCSMSplitRadius.x )
		return 1;
...
float4 PSMAIN_AMBIENT(float2 vRenderTargetCoord : TEXCOORD0) : COLOR
{
	float fDepth = abs( tex2D(GDepth, vRenderTargetCoord).x );

	float4 vPosInView = ReconstructPosFromDepth(vRenderTargetCoord, fDepth);

	float3 vLightInLocal = abs( mul(vPosInView, g_matWorldViewInv) );

	float3 vAttenuation = GetAttenuation(g_vAmbientLightRange.xyz-g_vAmbientLightRange.w, g_vAmbientLightRange.xyz, vLightInLocal, 1.0f);

	return float4(g_vLightColor, 0)
		* min( min(vAttenuation.x, vAttenuation.y), vAttenuation.z );
}
...
technique AmbientLightTechnique
{
	pass P0
	{
		PixelShader  = compile ps_3_0 PSMAIN_AMBIENT();
	}
}