...
float	g_fFalloffFar;	// = 1500.f
float	g_fFalloffNear;	// = 1200.f
...
float3 fetch_eye_nor( float2 uv)
{
	...
	// 원래는 노말의 Dir을 적용 하는 것이 정확히 맞는 것인데 무조건 -로 두는 것이 시각적으로 좋다. 왤�?대체 ㅠㅠ
...
float4 PSmainLowQuality( in float2 screenTC : TEXCOORD0 ) : COLOR
{
	...
	float fFactor = 1;
  	...
  	const float fReciprocalDist	= 1.0f / ( g_fFalloffFar - g_fFalloffNear);
  	/// 1: 림가득 0: 림없음
 	fFactor = 1 - saturate( ((P.z) - g_fFalloffNear) * fReciprocalDist);
 	[branch]
 	if( fFactor <= 0)
 		return 1;
