...
float4 BlurPS(in float2 screenTC : TEXCOORD0) : COLOR
{
    ...
    float4 vPointValue = tex2Dproj(LightMap, float4(screenTC, 0, 1));
    ...
    for (int i = 0; i < NUM_SAMPLE * 2; i++)
    {
        ...
        float4 vSamValue = tex2Dproj(LightMap, float4(vSamTC, 0, 1));
