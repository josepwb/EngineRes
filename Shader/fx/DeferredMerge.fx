...
float4 PSLightingOnly( in float2 uv : TEXCOORD0 ) : COLOR
{
	if( ReadDepth( uv) <1)
		return float4(ReadLight(uv).rgb, 1.0f);
	else
		return float4(ReadDiffuse(uv), 1.0f);
}

float4 PSPixelProfile( in float2 screenTC : TEXCOORD0 ) : COLOR
{
	float4 vLeft = tex2D( DiffuseSampler, screenTC);
	
	if( vLeft.x <= 0 )
		return float4(1,1,1,1);
	else if( vLeft.x <= 0.1f )
		return float4( 0, 0, vLeft.x * 10, 1);
	else if( vLeft.x <= 0.2f )
		return float4( 0, ( vLeft.x - 0.1f ) * 0.5 + 0.5, 0, 1);
	else if( vLeft.x <= 0.3f )
		return float4( ( vLeft.x - 0.2f ) * 0.5 + 0.5, 0, 0, 1);
...
technique LightingOnly
{
    pass P0
    {
		PixelShader  = compile ps_3_0 PSLightingOnly();
    }
}

technique PixelProfile
{
    pass P0
    {
		PixelShader  = compile ps_3_0 PSPixelProfile();
    }
}
